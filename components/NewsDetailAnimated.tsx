...
interface NewsDetailAnimatedProps {
    newsItem: {
        title: string;
        date: string;
        event_period?: string;
...
export default function NewsDetailAnimated({ newsItem }: NewsDetailAnimatedProps) {
    const [selectedImage, setSelectedImage] = useState<string | null>(null);
    const [selectedSet, setSelectedSet] = useState("relevantImages");
    const [showFullImage, setShowFullImage] = useState(false);
    const shareUrl = typeof window !== "undefined" ? window.location.href : "";
    const [searchQuery, setSearchQuery] = useState("");
    const currentIndex = newsData.findIndex(news => news.slug === newsItem.slug);
    const prevNews = newsData[currentIndex - 1] || null;
    const nextNews = newsData[currentIndex + 1] || null;
    console.log(newsItem, "newsItem")

    const handleShare = (platform: string) => {
        const urlEncoded = encodeURIComponent(shareUrl);
        const textEncoded = encodeURIComponent(newsItem.title);
    ...
    useEffect(() => {
        document.title = newsItem
            ? ` Events | ${newsItem.title} | Sona School of Business and Management`
        ...
        <>
            ...
            >
                <div className="pt-8 ">
                    ...
                    >
                        <span className="text-lg">←</span> Back to All News
                    </Link>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    {/* Sidebar */}
                    <aside className="lg:col-span-1 flex flex-col space-y-6 h-auto lg:h-[500px]">
                        {/* Search */}
                        ...
                        {/* List (show only 3) */}
                        <div className="space-y-6 overflow-y-auto pr-2 flex-1 max-h-full">
                            ...
                            {filteredNews.length > 3 && (
                                <Link
                                    href="/news" // 👈 redirect target page
                ...
                >
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-gray-600">
                        📅{" "}
                        {newsItem.event_period ? (

                            <span>{newsItem.event_period}</span>
                        ) : newsItem.date ? (
                ...
                {/* Relevant Images Grid */}
                {newsItem.relevantImages && newsItem.relevantImages.length > 0 && (
                    <motion.div
                    ...
                    >
                        {newsItem.relevantImages.map((img, index) => (
                            <motion.img
                                key={index}
                                src={img}
                                alt={`Related ${index + 1}`}
                                className="w-full h-40 sm:h-44 md:h-52 lg:h-60 object-cover rounded-lg shadow-md cursor-pointer"
                ...
                {newsItem.pressrealse && newsItem.pressrealse.length > 0 && (
                    <div className="mt-3">
                        ...
                        <motion.div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {newsItem.pressrealse.map((img, index) => (
                                <motion.img
                                    key={`press-${index}`}
                                    src={img}
                                    alt={`${newsItem.title} - press release ${index + 1}`}
                    ...
                    {selectedImage && (
                        ...
                        >
                            <div className="relative max-w-full max-h-full flex items-center justify-center">
                                ...
                                {/** Use gallery or press release based on selectedSet */}
                                {(() => {
                                    const currentSet =
                                        selectedSet === "gallery"
                                            ? newsItem.relevantImages ?? []
                                            : newsItem.pressrealse ?? [];

                                    const currentIndex = currentSet?.indexOf(selectedImage) ?? 0;
